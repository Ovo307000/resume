...
export function TechnologyTag({ name, size = 'small' }: TechnologyTagProps) {
  ...
  const color = getColorForTechnology(name);
  const url = getUrlForTechnology(name);
  ...
  return (
    <CustomChip
      ...
      animate={true}
      interactive={!!url}
      customBgColor={theme === 'dark' ? `rgba(${parseInt(color.slice(1, 3), 16)}, ${parseInt(color.slice(3, 5), 16)}, ${parseInt(color.slice(5, 7), 16)}, 0.2)` : `rgba(${parseInt(color.slice(1, 3), 16)}, ${parseInt(color.slice(3, 5), 16)}, ${parseInt(color.slice(5, 7), 16)}, 0.1)`}
